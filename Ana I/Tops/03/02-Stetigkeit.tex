...
\prop[Komposition von stetigen Abbildungen]{-1.4}{
...
\prop{-1.4}{
	Seien $X,Y$ metrische Räume, $f:X\rightarrow Y$ eine Abbildung. Seien $A_i\subset X,~a\leq i\leq n$, abgeschlossene Mengen mit $\bigcup\limits_{i=1}^nA_i=X$. Dann ist $f$ genau dann stetig, falls für alle $f|_{A_i}:A_i\rightarrow Y$ stetig sind.
}
\defi[Gleichmäßige Stetigkeit]{-1.4}{
	Seien $E,F$ metrische Räume. Sei $f:E\rightarrow F$ eine Abbildung. Dann heißt $f$ \textbf{\textit{gleichmäßig stetig}}, falls
...
\bem{-0.65}{
	\begin{compactenum}
		\item $\delta$ hängt nicht von $x$ ab
		\item gleichmäßig stetig $\Rightarrow$ stetig, aber nicht umgekehrt
		\item ist $E$ kompakt folgt aus Stetigkeit auch gleichmäßige Stetigkeit
	\end{compactenum}
}
\prop{-1.175}{
	Seien $X,Y,Z$ metrische Räume, $f:X\rightarrow Y,~g:Y\rightarrow Z$ gleichmäßig stetige Abbildungen. Dann ist $g\circ f$ gleichmäßig stetig.
}