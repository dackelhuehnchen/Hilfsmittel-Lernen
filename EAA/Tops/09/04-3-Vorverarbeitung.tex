...
\begin{itemize}[itemsep=-2pt]
	...
	\item Berechnung von $s_P^{pref}$:
		...
		\vspace*{-0.5\baselineskip}\item[$-$] somit kann $s_P^{pref}$ in $\BigO(m)$ berechnet werden (\algo{Good Suffix Regel (Prefix-Fall)}{arg2})%TODO Algorithmus
		\end{enumerate}
...
\begin{itemize}
	\item[]
		\begin{itemize}
			\item somit gilt $s_P^{match}\geq m-i+1+q$ (\algo{Good Suffix Regel (Matching-Fall)}{arg2})%TODO Algorithmus
